...
\title{Limits and Risks of Generative Scientific Discovery}
...
%Assessing the limitations and risks from generative models used for scientific discovery, I focus on three areas: \textbf{i)} innovation of novel generative architectures; \textbf{ii)} responsible individual use of generative models; \textbf{iii)} how networks of scientists use generative models for discovery. 

Generative machine learning models have produced non-trivial new scientific knowledge.  Are there limits to the production of such knowledge? What risks, if any, might the particular use of generative tools pose to the scientific method?  I address these questions by examining three aspects of generative models:  \textbf{i)} current directions of innovation in generative model architectures used for discovery; \textbf{ii)} use of generative models by individuals and in scientific networks; \textbf{iii)} the evaluation of generative scientific discoveries. This document briefly outlines the project and my methodology, provides writing samples, and other relevant experience.\footnote{No part of this document was artificially generated.}

%Respectively, dialectical argumentation theory in the context of architectural innovations, including important debates from philosophy of science to characterize potential new knowledge derived from generative models.
...
\noindent\textbf{Generative Architectures:} Widely available generative language models (LLMs) have problems, like confabulation, which architectural innovations can mitigate.  A recent example of architectural innovation \citep{FunSearch2024} shows how, in the context of (successful) discovery, confabulations can be overcome.  What kinds of effective architectural innovations should we expect in the future, and are there limits to what we could discover even with an ideal generative model?  Are there potential risks specific to scientific use of generative models in the context of discovery?
...
Specialized and narrow ANN models are of a wholly different character from recently popular generative large language models (LLMs). The way each type are used also differ.  The former example are used as tools of discovery much like other scientific models or other computational methods.  Generative language models are used more like oracles. Use of LLMs, and multi-modal expansions of LLMs, can affect many aspects of the work flow and methodology of scientists. When prompted, and to varying degrees of success, these models can generate code, text, arguments, diagrams, graphs, images, and sounds.  They can summarize text, translate text, and perform literature reviews. They can also suggest apparently new theories or possible experiments, or even attempt metaphysical speculation.  

%If many scientists incorporate generative models into their methodology, the potential resulting generative `pollution' affects the economic trade offs for evaluating .  for scientific discovery in areas like fundamental physics (or in borderline areas which may be considered metaphysics by some), there are major resource costs (e.g. time) to checking and testing generated claims. Generative models are beginning to flood the zone of critical intellectual effort by humans.
...
We must consider what it means for generative AI to be used with the aim of making new scientific discoveries.  Even if we might gain some justificatory support by using new dialectical architectures for such discoveries, we should consider whether there may be limits to the kind of scientific knowledge obtainable.  For example, it seems that for certain contexts of discovery, a generative model might produce hypothetical claims using theoretical terms which may or may not be observable.  We know that artificial systems do not interact with the world in the same way that humans do, and if the training data is derived from human interactions with the world, AI models can only be considered to `live' in a derivative `world'.
...
One very interesting new model particular to scientific discovery with a novel architecture was recently published, called FunSearch.  \citep{FunSearch2024}  Again, an LLM was combined with other components to mitigate confabulations.  The FunSearch results even provided a form of explanation that philosophers of science might recognize. A mathematical result was obtained and communicated in the form of a (low $K$ complexity) program. This ended up providing a positive feedback loop with the researchers who used such a clear program of results to further discover even better solutions to the problem. Are there limits to such a collaborative process of discovery?\\
...
\noindent\textbf{Risk Framework:} There might be sophisticated components of a generated theory (linguistic, mathematical, or otherwise) which exceed the capacity of networks of scientists to efficiently evaluate.  I argue such a potential situation represents a risk from generative AI, and it may happen in an accumulative manner (as opposed to catastrophic) as outlined in \cite{Kasirzadeh2024WIP}  Following her arguments, I also view the current state of AI in our civilization through the lens of complex systems theory.  

Consider a phase space, in which each point represents the total state of every human with agency, and every relevant factor of human civilization.  This complex system has, and will have, a \emph{trajectory} in phase space.  In this space there might be `valleys' and `peaks' (attractors and repellors) which represent the grain or currents due to various mechanisms and processes, incentives and disincentives, positive and negative feedback loops, and structural constraints in the world of the system.  What constraints or influences do generative models impart on trajectories in such an abstract space?  I argue that \textbf{i)-iii)} are crucial components in the complex system where it is desirable to build resilience and avoid accumulative risk. Philosophers engaging in interdisciplinary work are well suited to make meaningful contributions towards mitigation of accumulative risk in all three areas. 
...
% \begin{itemize}
%     \item \textbf{Large Language Models.}  I swear I will \textbf{not} use LLMs to summarize literature, write text for my papers, or to help construct ``novel'' arguments or substitute for my own effort and cognition in any way. Any use will be purely for research purposes, in accordance with agreed upon guidelines.  If there are no guidelines, I would work to help create them. 
...
%     %\item \textbf{Teaching.}  No use of generative models allowed by students.  Consistent with the methods at the MCMP, when feasible and applicable I believe formal methods and basic programming should be introduced to philosophy students.  See \href{https://github.com/CameronBeebe/Probability_Intro/blob/main/Expected_Value.ipynb}{this jupyter notebook on basic expected value as an example of an assignment format.}  I would be interested in teaching a course based loosely on \cite{Ashby1958} \emph{Introduction to Cybernetics}, among other subjects.
...
%\section{Academic environment}

%Most courses I took involved some math, programming, and formal methods to varying degrees at LMU and through the Munich Center for Mathematical Philosophy (MCMP) and Graduate School of Systemic Neurosciences (GSN).  Interdisciplinary collaboration was encouraged.  \\

%\noindent The philosophy curriculum at the MCMP emphasized a variety of basic formal methods. \\

%: probability theory, information theory, formal logic, and basic linear algebra and calculus if working in more technical areas in philosophy of physics and interdisciplinary AI theory.  \\
...
% \begin{multicols}{2}
% \begin{itemize}
%     \item \textbf{Measurement and Recording}: PH; total dissolved solids (tds) or parts per million (ppm); water temperature; room humidity and temperatures; nutrient solution amounts; plant weights; 
