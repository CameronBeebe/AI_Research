...
\begin{itemize}
    ...
    \item \textbf{Networking.} I spent almost a decade associated with the Munich Center for Mathematical Philosophy.  I am also working on a project as an external member.  I also created The SciPhi Initiative, LLC, and have used this organization for tutoring and outreach.  I will continue to use social media, primarily those where tech and AI industry experts are also present, to engage with respect to research and ethics goals.
...
We have some idea now, after more than a decade of widespread social media use by humans, how populations might be affected by algorithmic content pushing.  Feedback loops provide information to social media companies to adjust their algorithms with the aim of keeping users neurochemically hooked and coming back for more content, hoping to serve more ads.  Supercomputers are employed in order to do large amounts of mathematics (e.g. matrix multiplication, chain rule) in the optimization process.

With the advent and widespread availability and use of generative large language models, there is now a new optimization loop which reinforces through human feedback a different class of models.  These models aren't necessarily optimized in order to serve content, but to create linguistic content that satisfies in some way a population of human users.  Supercomputers are dedicated to training and improving these models, and also to serve artificial linguistic content to large populations of humans.

There is a large amount of literature about bias in artificial neural network models.  However, in my opinion this literature focuses mainly on the ethics and corrective procedures in order to balance (or even remove the bias totally, which under this understanding [SOURCE] is not particularly plausible) bias, which I think is missing a much larger potential issue that tech and AI ethicists should be aware of.  

Even if everyone agreed on criteria for an ``unbiased'' model, and everyone agreed to use it, there are fundamental epistemological limitations about whether we will be able to understand when the models are hallucinating or nudging in nuanced ways that affect influential actors in society, which in turn affect the trajectory of science or politics in our civilization.
...
\noindent\textbf{What effect might widespread AI nudges have on this trajectory?}

In a preprint manuscript, Atoosa Kasirzadeh makes a very similar and related point she calls the \emph{accumulative} risk of AI. \citep{Kasirzadeh2024WIP}  I would argue that central to this accumulative risk is the relationship populations of humans have to widely available generative artificial systems.  Small mistakes and misuses can add up: 
