...
\title{Generative AI in Scientific Discovery: \\ Research Plan and Experience}
...
This research plan proposes to philosophically address and contextualize the current (and perhaps future) capability of generative artificial intelligence with respect to scientific discovery.  In particular, while acknowledging limited successes, I would like to focus on potential problems and limits to the use of generative models for scientific discovery.  Following the brief outline of the topics I will work on, I also lay out a proposed methodology and summarize my background which is suitable for successful research and publication on this topic.
...
With respect to the generative AI models themselves, I think philosophers should pay particular attention to what can be called \emph{dialectical architectures.}  There are strong reasons that such architectures may be able to meaningfully mitigate some of the widely acknowledged issues (e.g. bias, confabulation or hallucination, explainability and interpretability). In turn, mitigating these low level issues may also mitigate accumulative risk in the individual and scientific network components of AI use in our civilization.  

% Perhaps they can also help to encourage non-Oracle use of these models as well.

This is especially important if lay individuals or scientists utilize generative AI with the aim of producing new knowledge and scientific discoveries.  Even if we might gain some justificatory support by using dialectical architectures for such discoveries, there are likely to be limits to the kind of scientific knowledge obtainable.  For such limits I suspect that debates around structural realism will have important lessons which transfer over to the discussion about scientific discoveries from generative AI.

For example, it seems that for certain contexts of discovery, a generative model might produce hypothetical claims using theoretical terms which may or may not be observable.  Can we evaluate such theories on the same footing as other human-derived theories?  Even though we know generative models might be prone to confabulation, but could a dialectical architecture mitigate that worry enough that we would be confident to commit resources to investigating such a theory?  Such commitment, to me, seems to contain a potential to introduce additional civilizational accumulative risk from AI.
